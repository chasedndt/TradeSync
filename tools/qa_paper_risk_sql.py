...
* Session A runs one script inside one transaction: a throwaway schema, then
  023, 026 and 031 UP, 031 DOWN, 031 UP again, then the seeds, the append-only
  triggers, the check constraints, and the reconciliation, mark and gap
  statements state-api issues (imported from its modules and prepared as they
  are) against seeded rows. It then takes advisory lock 230914, the entry lock,
  holds it, and rolls back. ON_ERROR_STOP ends the session, uncommitted, on the
  first failed check.
* Session B, a separate connection per step, must see A holding the lock, fail
  to take it, block while waiting for it, acquire it only after A's rollback,
  and finally find the throwaway schema gone.

Unqualified names resolve only inside the throwaway schema (``SET LOCAL
search_path``), so nothing in the public schema is read or written; the
advisory lock is the only shared object touched, for a few seconds.

    python tools/qa_paper_risk_sql.py                  # docker exec into tradesync-full-postgres-1
    python tools/qa_paper_risk_sql.py --local 127.0.0.1:55432:qa:postgres --psql-bin "C:/Program Files/PostgreSQL/17/bin/psql.exe"
"""
...
from app.paper_account_store import STATES_AT_SQL  # noqa: E402
from app.paper_reconciliation_store import GAP_PAIRS_SQL, LAST_SEEN_SQL, LATEST_STATES_SQL, UPSERT_GAP_SQL  # noqa: E402
...
def check(condition: str, failure: str) -> str:
    return f"DO $$ BEGIN IF NOT ({condition}) THEN RAISE EXCEPTION 'QA FAIL: {failure}'; END IF; END $$;"
...
def session_a_script(schema: str, application: str, hold_s: float) -> str:
    ...
    ledger = "INSERT INTO paper_account_ledger (id, kind, position_id, occurred_at, amount_usdc, gross_pnl_usdc, fees_usdc, funding_usdc, balance_after_usdc) VALUES (gen_random_uuid(), "
    return "\n".join([
        ...
        """CREATE FUNCTION pg_temp.qa_refused(statement text, expected text) RETURNS void LANGUAGE plpgsql AS $f$
BEGIN
  BEGIN
    EXECUTE statement;
...
END $f$;""",
        ...
        "INSERT INTO opportunities VALUES ('11111111-1111-4111-8111-111111111111'), ('22222222-2222-4222-8222-222222222222');",
        ...
        ledger + f"'realised', '{P2}', clock_timestamp(), -1.5, -1, 0.4, 0.1, 9998.5);",
        refused(ledger + f"'realised', '{P1}', clock_timestamp(), 5, 1, 0, 0, 1)", "23514"),
        refused(ledger + f"'realised', '{P2}', clock_timestamp(), -1.5, -1, 0.4, 0.1, 9997)", "23505"),
        ...
        "\\echo PASS ledger: append-only against UPDATE, DELETE and TRUNCATE; one capital row; realised amount must equal gross less fees and funding; one entry per position",
        ...
        "\\echo PASS state-api statements as prepared: gap upsert, one 95-second gap, last observation, lifecycle candidates, boundary states",
