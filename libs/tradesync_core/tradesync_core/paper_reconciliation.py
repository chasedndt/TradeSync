...
Every stored position must equal the latest state its own lifecycle events
recorded, and the ledger and stored balances must equal what the closing events
imply (``paper_account_ledger.compare``). A pause between consecutive
observations of a position longer than the lifecycle's own 45-second latch is
reported with its start and end; an open position not observed for that long is
an ongoing gap with no end yet. Nothing here writes an observation, so a gap is
never filled in.
"""
...
from tradesync_core.paper_account_ledger import capital_entry, compare, money, realised_entry

GAP_THRESHOLD_S = 45.0
...
def lifecycle_order(state: Mapping[str, Any]) -> tuple[int, float, int]:
    """The lifecycle's own order for states of one position: observations, quote time, closed last.

    The write clock is not trusted for this: two transactions can record
    ``created_at`` in the opposite order to the one in which they held the row.
    The state-api store sorts with the same key in SQL.
    """
    return (int(state.get("observations") or 0), float(state.get("last_quote_time") or 0.0), 1 if state.get("status") == "closed" else 0)
...
def account_mismatches(account: Mapping[str, Any] | None, ledger_rows: Iterable[Mapping[str, Any]],
                       closed: Iterable[tuple[Any, Mapping[str, Any]]], peaks: Iterable[Any]) -> list[dict[str, Any]]:
    if account is None:
        return [{"code": "ACCOUNT_MISSING", "detail": "No paper account row; balances cannot be checked"}]
    issues: list[dict[str, Any]] = []
    realised = []
    for position_id, state in closed:
        try:
            realised.append(realised_entry(position_id, state))
        except ValueError as exc:
            issues.append({"code": "CLOSED_POSITION_UNREADABLE", "position_id": str(position_id), "detail": str(exc)})
    realised.sort(key=lambda e: (e.occurred_at, e.position_id))
    expected = [capital_entry(account["starting_capital_usdc"], 0.0), *realised]
    unreadable = {i["position_id"] for i in issues}
    issues += [i for i in compare(ledger_rows, account, expected)
               if not (i["code"] == "LEDGER_UNEXPECTED_ENTRY" and i.get("position_id") in unreadable)]
