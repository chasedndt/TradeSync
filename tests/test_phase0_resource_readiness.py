...
def test_readiness_audit_is_paper_only_and_fail_closed() -> None:
    completed = subprocess.run(
        ...
        text=True,
        capture_output=True,
        # Ollama/WSL probes can legitimately take over 45 seconds when their
        # runtimes are stopped. The audit still fails closed; the test must
        # wait long enough to inspect that result instead of timing out first.
        timeout=90,
    )
