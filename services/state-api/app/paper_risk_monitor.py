...
async def close_for_kill(pool, fetch_book: BookFetcher) -> None:
    async with pool.acquire() as conn:
        kill = await control.kill_state(conn)
    if kill is None or not kill["active"]:
        status["kill_pending"] = []
        return
    result = await paper_kill_switch.close_open_positions(pool, fetch_book, operator=kill["operator"], reason=kill["reason"])
...
async def tick(pool, fetch_book: BookFetcher, now_s: float | None = None) -> None:
    now_s = time.time() if now_s is None else now_s
    await close_for_kill(pool, fetch_book)
...
def loop(state, fetch_book: BookFetcher):
    async def run() -> None:
        await asyncio.sleep(5)  # the startup reconciliation goes first
        while True:
            if state.pool is not None:
                try:
                    await tick(state.pool, fetch_book)
