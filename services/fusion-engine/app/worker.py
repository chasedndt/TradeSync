...
# Account equity assumption for margin utilization calculation.
# Must match the assumption used elsewhere (state-api preview_action uses $50k).
ACCOUNT_EQUITY_USD = float(os.getenv("ACCOUNT_EQUITY_USD", "50000.0"))
# Short timeout — positions fetch must not stall the scoring pipeline.
POSITIONS_FETCH_TIMEOUT = float(os.getenv("POSITIONS_FETCH_TIMEOUT", "1.5"))
...
async def fetch_market_snapshot(symbol: str, venue: str = "hyperliquid") -> Optional[Dict[str, Any]]:
...
async def fetch_exposure_data(symbol: str) -> Optional[Dict[str, Any]]:
    """
    Fetch current position exposure from state-api for EnhancedScorer.

    Builds the exposure_data dict expected by EnhancedScorer._compute_exposure_penalty():
        {
            "by_symbol": {symbol: size_usd, ...},  # USD notional per symbol (all venues combined)
            "margin_utilization": float             # total_notional / ACCOUNT_EQUITY_USD, capped at 1.0
        }

    Returns None on any failure — the caller must treat None as "no exposure data available"
    and fall back gracefully (exposure penalty = 0).

    Phase 3F-1: This function was the missing piece — previously exposure_data was always
    passed as None to compute_enhanced_score(), making the exposure penalty permanently 0.
    """
    try:
        async with httpx.AsyncClient() as client:
            resp = await client.get(
                f"{STATE_API_URL}/state/positions",
                timeout=POSITIONS_FETCH_TIMEOUT
            )

        if resp.status_code != 200:
            print(f"[Worker] /state/positions returned HTTP {resp.status_code} — exposure_data=None")
            return None

        positions = resp.json()

        # Empty positions list is valid: no exposure anywhere.
        if not positions:
            return {"by_symbol": {}, "margin_utilization": 0.0}

        # Sum USD notional per symbol across all venues.
        by_symbol: Dict[str, float] = {}
        for pos in positions:
            sym = pos.get("symbol", "")
            size_usd = abs(float(pos.get("size_usd", 0)))
            if sym:
                by_symbol[sym] = by_symbol.get(sym, 0.0) + size_usd

        total_notional = sum(by_symbol.values())
        margin_utilization = min(total_notional / ACCOUNT_EQUITY_USD, 1.0)

        symbol_usd = by_symbol.get(symbol, 0.0)
        print(
            f"[Worker] Exposure fetched: {symbol}=${symbol_usd:,.0f} "
            f"total=${total_notional:,.0f} margin_util={margin_utilization:.1%}"
        )
        ...
        print(f"[Worker] Failed to fetch exposure data: {e} — falling back to exposure_data=None")
        return None
...
async def process_message(msg_id, data):
    ...
    try:
        ...
        # Phase 3F-1: Fetch live position exposure for concentration/margin penalty.
        # Returns None on failure — EnhancedScorer skips the exposure penalty when None.
        exposure_data = await fetch_exposure_data(symbol)
