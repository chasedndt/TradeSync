...
import { REASON_LABELS, REASON_ORDER, count, duration, percent } from './format'
...
export function HealthStrip({ overview }: { overview: UseQueryResult<RegimeLabOverview, Error> }) {
  ...
  const marketLive = health.market_data.status === 'live'
  ...
  return (
    <section className={`panel ${styles.strip}`} aria-label="Evidence health">
      ...
      <div className={styles.stats}>
        <Stat label="Live inputs" value={`${health.fresh_features}/${health.feature_count}`} sub="fresh readings only" tone={health.fresh_features > 0 ? styles.good : styles.warn} />
        <Stat label="Scoring coverage" value={percent(evaluation.data_coverage)} sub={`${scoring} scoring ${scoring === 1 ? 'feature' : 'features'} ready`} />
        ...
        <Stat
          ...
          sub={marketLive ? `${count(health.market_data.observation_count)} observations · read ${readAt} UTC` : health.market_data.reason ?? 'no reason given'}
          tone={marketLive ? styles.good : styles.bad}
        />
      </div>
      <ul className={styles.reasons} aria-label="Why coverage is what it is">
        {REASON_ORDER.map((reason) => {
          ...
          return (
            <li key={reason} className={`${styles.reason} ${n === 0 ? styles.zero : ''}`} data-reason={reason}>
        ...
        {health.feeds.map((feed) => (
          <li key={feed.feed} className={styles.feed} data-status={feed.status}>
            ...
            <span className={styles.feedAge}>
              {feed.observed === 0 ? 'no reading' : `newest ${duration(feed.newest_age_ms)} old · stale after ${duration(feed.stale_after_ms)}`}
            </span>
          </li>
        ))}
      </ul>
