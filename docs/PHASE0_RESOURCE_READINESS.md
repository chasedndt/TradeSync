...
> 2026-08-24 provenance update: the authoritative GitHub repository and remote `tradesync-hyperliquid-only` base `7d6f671` were recovered into an isolated E: worktree. The old relocated directory remains an orphaned evidence/source snapshot and was not rewritten. Current startup gates must be evaluated from the recovered branch after integration; live execution remains prohibited.
